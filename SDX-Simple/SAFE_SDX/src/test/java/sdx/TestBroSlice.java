...
public class TestBroSlice extends SliceBase {
  ...
  public String sliceName() { return "t3"; }
  ...
  public void setUpFlow() throws SampleSlice.SliceBaseException {
    ...
    threads.add(new Thread(() -> {
        try {
          ...
          execOnNode(flow, "while [ \"`fuser /var/lib/dpkg/lock`\" != \"\" ]; do sleep 1; done"); // Make sure apt install is do { ret = execOnNode(flow, "wget -nv http://download.opensuse.org/repositories/network:bro/xUbuntu_17.04/Release.key -O Release.key"); } while (!ret);
          ...
          execTillSuccess(flow, "apt install -y bro");
          execOnNode(flow, "echo 'export PATH=$PATH:/opt/bro/bin' >> ~/.bashrc");

          execTillSuccess(flow, "apt install -y openvswitch-switch");
          execTillSuccess(flow, "apt install -y iperf3");
          /*
          execTillSuccess(flow, "apt install -y unzip");
          execTillSuccess(flow, "apt install -y git cmake make gcc g++ flex bison libpcap-dev libssl-dev python-dev swig zlib1g-dev");
