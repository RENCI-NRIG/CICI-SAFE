...
public class SdxExogeniClient extends SliceCommon {
    ...
    public void runExec(String operationParams) throws Exception {
        try {
            loadSlice();
        } catch (Exception e) {
            e.printStackTrace();
        }
        processCmd(operationParams);
        logger.info(logPrefix + "XXXXXXXXXX Done XXXXXXXXXXXXXX");
    }
    ...
    public boolean ping(String nodeName, String ip) {
        ...
        String res = serverSlice.runCmdNode("ping  -c 1 " + ip, node.getName(), false);
        logger.debug(res);
    ...
    public boolean ping(String nodeName, String ip, int num) {
        ...
        ComputeNode node = serverSlice.getComputeNode(nodeName);
        String res = serverSlice.runCmdNode(String.format("ping -i 0.01 -c %s %s", num, ip), node.getName(),
                false);
        logger.debug(res);
    ...
    public String traceRoute(String nodeName, String ip) {
        ...
        ComputeNode node = serverSlice.getComputeNode(nodeName);
        String res = serverSlice.runCmdNode("traceroute -N 1 " + ip, node.getName(), true);
        logger.debug(res);
        return res;
    }
