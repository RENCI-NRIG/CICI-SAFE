...
abstract class Term extends Cloneable with LazyLogging {
  def ref = this
  ...
  final def matches(that: Term, trail: Trail): Boolean = {
    ...
    trail.unwind(oldtop)
    if(ok == false) {
      logger.info(s"${this} doesn't match ${that}")
    }
    ok
  }
...
object Term extends LazyLogging {
  ...
  */
  ...
  /* 
   * @Developer API
   * Inspect a clause by printing out each of its terms 
   */
  def printClause(terms: List[Term]): String = {
