...
abstract class Term /*extends Marshaler[Term]*/ { // Not sealed as we would expand further from slang
  ...
  def firstParam: String = ""

  // useful at slang level
  def bind(args: Map[StrLit, Term]): Term = {
    var _term = this
    args.foreach {
      case (varName, varValue) =>
        _term = _term.bind{v => if(v.name == varName.name) varValue else Variable(v)}
    }
    _term
  }
  ...
  */
  ...
  override def toString(): String = {
    def encodingStr(id: String, enc: Encoding, annotate: Boolean = false): String = (enc, annotate) match {
      ...
      case  (Encoding.AttrBase64, true)         => s"'$id'"                            // Base64 + no index
      case  (Encoding.IndexAttrBase64, true)    => s"'$id'"                            // Base64 + indexable
    ...
    //println("tpe=" + tpe + "  attrName=" + attrName + "   indexAndEncode=" + indexAndEncode)
  ...
  def secondaryIndex(): StrLit = StrLit(id.name + arity + firstParam) // index on principal functor, arity and the first parameter.
...
trait VariableLike extends Term {
  ...
  override def secondaryIndex(): StrLit = StrLit("nil")
...
trait StructureLike extends Term {
  ...
  override val firstParam: String = {
    if(terms.length > 1) terms(1).id.name  // terms(0) is the speaker; we don't index with that
    else if(terms.length == 1) terms(0).id.name // only one term, index using that
    else ""
  }
  ...
  def compare(): Boolean = (id, terms) match {
    ...
    case (StrLit("_in"), head +: tail +: Nil)       => (head, tail) match {
      case (Structure(StrLit("_ipv4"), ip +: Nil, _, StrLit("address"), _), Structure(StrLit("_ipv4"), cidr +: Nil, _, StrLit("block"), _)) => 
        val subnet = getSubnetUtils(cidr.id.name)
        subnet.getInfo.isInRange(ip.id.name)
      case (Structure(StrLit("_ipv4"), cidr0 +: Nil, _, StrLit("block"), _), Structure(StrLit("_ipv4"), cidr1 +: Nil, _, StrLit("block"), _)) => 
        val subnet0 = getSubnetUtils(cidr0.id.name)
        val subnet1 = getSubnetUtils(cidr1.id.name)
  ...
  def getIdWithType(): String = {
    ...
    else s"${attrName.name} #${tpe.name} -> ${id.name}"  // credential 
  }
  override def toString(): String =  (Config.config.saysOperator, id, terms) match {

    // == support for lists in slang Start ==/ // TODO: move this to slang
    case (_, StrLit("_seq"), xterms) if (xterms.last == Structure(StrLit("nil"), Nil)) =>
    ...
    case (_, StrLit("_ipv4"), xterms) if xterms.length >= 1 =>
      "ipv4" + "\"" + xterms.head.id.name + "\""
  ...
  override def toStringCompact(speaker: String): String = (id, terms) match {
    ...
    case (StrLit("_ipv4"), xterms) if xterms.length >= 1 =>
      "ipv4" + "\"" + xterms.head.id.name + "\""
    ...
    case (StrLit("_fact"), subject +: Constant(value, _, _, _) +: Nil)              => 
      subject + ": " + value.name
    case (_, subject +: tail)                                              => 
      //throw new UnSafeException(s"subject != speaker;  subject=${subject}    speaker=${speaker}")
  ...
  override def bind(f: StrLit => Term): Term = (id, terms) match {
    case (StrLit("_interpolate"), Constant(body, attrName, tpe, enc) +: Constant(termSeq, _, _, _) +: xterms) =>
      ...
      if(!globalVariablesGround) { this.copy(terms = terms.map(_.bind(f))) }
        ...
        //val rs         = Constant(bindedStr, attrName, tpe, Encoding.AttrLiteral) // bindedStr may contain local variables --- hence, leave the string with double quotes for possible another interpolation during inference
    ...
    case _   => //if(id.name == "inferSet") terms.foreach(t => println("[slog SetTerm] inferset term " + t + "   term.id " + t.id + "  t.getClass " + t.getClass + "  t.bind(f) " + t.bind(f) )); 
      this.copy(terms = terms.map(_.bind(f)))
  }
}
...
trait TermLike { // For reusing at higher layer (slang)
  ...
  private[safe] def mguRegex(id: StrLit, term: Term, lhs: Seq[Term], rhs: Seq[Term], unifier: Term => Term): Option[Term => Term] = {
    val regexInStr = Term.stripDepth(id.name.substring(1)).name   // Need to strip the trailing depth before use
    // println(s"[slog Term mguRex] regexInStr: $regexInStr;  term.id.name: ${term.id.name}")
    val underlying = s"""$regexInStr""".r
  ...
  private[safe] final def mguSeq(lhs: Seq[Term], rhs: Seq[Term], unifier: Term => Term): Option[Term => Term] = (lhs, rhs) match { 
    ...
    //case (term +: tail1, (v @ Variable(id, _, _, _)) +: tail2) if(v.simpleName.name != "Self" || (v.simpleName.name == "Self" && term.isInstanceOf[Variable] && term.asInstanceOf[Variable].simpleName.name == "Self")) =>   // Qiang: Wrong filter if(id.name != "Self") =>
    case (term +: tail1, (v @ Variable(id, _, _, _)) +: tail2) if(v.simpleName.name != "Self" || (term.isInstanceOf[Variable] && term.asInstanceOf[Variable].simpleName.name == "Self")) =>
      if(id.name.startsWith("^")) { // regex
	mguRegex(id, term, tail1, tail2, unifier)
      } else {
        //println(s"[slog term] Unify1 ${lhs.head} and ${rhs.head}  (${lhs.head.id.name} and ${rhs.head.id.name})   v.simpleName.name=${v.simpleName.name}")
    ...
    //case ((v @ Variable(id, _, _, _)) +: tail1, term +: tail2) if(v.simpleName.name != "Self" || (v.simpleName.name == "Self" && term.isInstanceOf[Variable] && term.asInstanceOf[Variable].simpleName.name == "Self")) => // Qiang: wrong filter if(id.name != "Self")  =>
    case ((v @ Variable(id, _, _, _)) +: tail1, term +: tail2) if(v.simpleName.name != "Self" || (term.isInstanceOf[Variable] && term.asInstanceOf[Variable].simpleName.name == "Self")) => 
      if(id.name.startsWith("^")) { // regex
	mguRegex(id, term, tail1, tail2, unifier)
      } else {
        //println(s"[slog term] Unify2 ${lhs.head} and ${rhs.head}  (${lhs.head.id.name} and ${rhs.head.id.name})   v.simpleName.name=${v.simpleName.name}")
	val r = subst(id, term)_ compose unifier // given two partial functions, f and g, compose will return g(f(x))
	mguSeq(tail1.map(r), tail2, r)  // term isn't a varialbe; mguSeq(tail1.map(r), tail2.map(r), r, envContext)
      }

    // constants must match
    case (Constant(id1, _, _, _) +: tail1, Constant(id2, _, _, _) +: tail2) =>
      if (id1 == id2) { 
        //println(s"[slog term] Unify3 ${lhs.head} and ${rhs.head}  (${lhs.head.id.name} and ${rhs.head.id.name})")
        mguSeq(tail1, tail2, unifier) 
      }
      else None
    ...
    case (_, _) => 
      //println(s"[slog term] Unify4 ${lhs.head} and ${rhs.head}  (${lhs.head.id.name} and ${rhs.head.id.name})    ${lhs.head.getClass} and ${rhs.head.getClass}")
      None
  }
...
*/
  ...
  def dropStatement(stmt: Statement, program: ConcurrentHashMap[Index, Set[Statement]]): Unit = {
    val orgStatements: Set[Statement] = program.get(stmt.primaryIndex)   // need to drop statements on the secondaryIndex as well
    ...
    args.zipWithIndex foreach {
      case (arg: String, idx: Int) =>
        val substValue: String = argValues(idx) match {
          ...
          case s @ Structure(StrLit("_ipv4"), _, _, _, _)          => s.toString
          case other       => throw UnSafeException(s"Unbound variables found in interpolation: $other; $argValues")
        }
       _mutableStr = _mutableStr.replace(arg, substValue) // TODO: this is simpler than string interpolation albeit expensive
    }
