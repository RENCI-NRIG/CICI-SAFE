 ...
 */
class DifcBench(concurrency: Int, jvmmapFile: String, slangManager: SafelangManager) extends 
    SafeOperation with StateManagementHelper with HarnessPerfStatsController with LazyLogging {
  override val operators = Seq("grantTagAccess", "labelFile", "delegateTagAccess", "checkFileAccess")

  override val opcountMap = Map("grantTagAccess"->2, "labelFile"->1, "delegateTagAccess"->2, "checkFileAccess"->1)
