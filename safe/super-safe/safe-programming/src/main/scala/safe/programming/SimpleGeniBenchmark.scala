...
import safe.safelang.{Safelang, REQ_ENV_DELIMITER}
 ...
 */
class SimpleGeniBench(inference: Safelang, jvmmapFile: String) extends BenchCommons {
  ...
  def run(): Unit = {
    ...
    // format: [speaker]|[subject]|[object]|[bearerRef]
    ...
    var envs: Seq[String] = Seq(REQ_ENV_DELIMITER + piList(0).getPid + REQ_ENV_DELIMITER + REQ_ENV_DELIMITER + piList(0).getSubjectSetTokens(0), emptyEnvs, emptyEnvs) 
    ...
    envs = Seq(REQ_ENV_DELIMITER + userList(0).getPid + REQ_ENV_DELIMITER + REQ_ENV_DELIMITER + userList(0).getSubjectSetTokens(0), emptyEnvs, emptyEnvs)
    ...
    var env: String = REQ_ENV_DELIMITER + userList(0).getPid + REQ_ENV_DELIMITER + REQ_ENV_DELIMITER + userList(0).getSubjectSetTokens(0)
    args = Seq(sliceId)
