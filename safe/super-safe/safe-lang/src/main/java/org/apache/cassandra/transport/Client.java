...
public class Client extends SimpleClient
{
    ...
    private void startup() {
        try
        {
        	Message.Request startup = parseLine("startup");
        	Message.Response resp = execute(startup);
            System.out.println("-> " + resp);
            resp = execute(parseLine("QUERY CREATE KEYSPACE demodb WITH REPLICATION = { 'class' : 'NetworkTopologyStrategy', 'datacenter1' : 3 };"));
            System.out.println("-> " + resp);
            resp = execute(parseLine("QUERY USE demodb;"));
            System.out.println("-> " + resp);
        }
        catch (Exception e)
        {
        ...
        try
        {
        	Message.Response resp = execute(parseLine("QUERY USE demodb;"));
            System.out.println("-> " + resp);
        }
        catch (Exception e)
        {
        ...
        try
        {
        	Message.Response resp = execute(parseLine("QUERY CREATE TABLE test ("+
  "id varchar,"+
  "cert blob,"+
  "expdate timestamp,"+
  "issuedate timestamp,"+
  "PRIMARY KEY (id));"));
        ...
        try
        {
        	Message.Response resp = execute(parseLine("QUERY CREATE INDEX ON demodb.test(expdate);"));
            System.out.println("-> " + resp);
        }
        catch (Exception e)
        {
        ...
        try
        {
        	Message.Response resp = execute(parseLine("QUERY CREATE INDEX ON demodb.test(issuedate);"));
            System.out.println("-> " + resp);
        }
        catch (Exception e)
        {
    ...
    private void write(String id, String text) {
        try
        {
        	int issue = 1490369687 - rnd.nextInt(100000);
        	int exp = 1490369687 + rnd.nextInt(100000);
//        	String query = "QUERY INSERT INTO demodb.test (id,cert,issuedate,expdate)"+
//  "VALUES ('"+id+"', textAsBlob('"+text+"'), "+issue+", "+exp+");";
//        	System.out.println(query);
        	Message.Request req = new WriteMessage(id, text, exp, issue, QueryOptions.create(ConsistencyLevel.ONE, Collections.<ByteBuffer>emptyList(), false, -1, null, null, version));
    ...
    private void read(String id) {
        try
        {
        	...
        	//String query = "QUERY SELECT * from demodb.test where id='"+id+"'";
        	Message.Request req = new ReadMessage(id, QueryOptions.create(ConsistencyLevel.ONE, Collections.<ByteBuffer>emptyList(), false, -1, null, null, version));
        	Message.Response resp = execute(req);
        }
        catch (Exception e)
        {
    ...
    public void run() throws IOException
	{
		...
		for (int i = 0; i < num; i++) {
			String id = "a"+rnd.nextInt();
			list.add(id);
			write(id, text);
			if (i % 10000 == 0)
				System.out.println("Write " + i + " th statement");
		}
		System.out.println("Finish writing" + num + " statements");
		for (int j = 0; j < num; j++) {
			read(list.get(rnd.nextInt(num)));
			if (j % 10000 == 0)
				System.out.println("Read " + j + " th statement");
		}
		System.out.println("Finish reading" + num + " statements");
