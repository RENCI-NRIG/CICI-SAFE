...
class PolicyRoutingTable(dict):
    ...
    def add(self, dst_nw_addr, dst_vlan, gateway_ip, src_address=None,src_nw_addr=DEFAULT_ROUTE):
        ...
        if src_address is not None:
            ...
            if(src_nw_addr != DEFAULT_ROUTE):
              msg="Can't Specify Both address_id and source"
        ...
        table = self[key]
        added_route = table.add(dst_nw_addr, dst_vlan, gateway_ip, self.route_id, src_nw_addr)
...
class RoutingTable(dict):
    ...
    def add(self, dst_nw_addr, dst_vlan, gateway_ip, route_id,src_nw_addr=DEFAULT_ROUTE):
        err_msg = 'Invalid [%s] value.'
        ...
        print "parsed ip:%s netmask:%s"%(dst_ip,dst_netmask)

        if src_nw_addr == DEFAULT_ROUTE and self.src_address is None:
            src_ip = 0
            src_netmask = 0
        elif src_nw_addr != DEFAULT_ROUTE:
            src_ip, src_netmask,src_dummy = nw_addr_aton(
                src_nw_addr, err_msg=err_msg % REST_SOURCE)
        else:
            src_ip = src_address.nw_addr
            src_netmask = src_address.netmask
        ...
        # Check overlaps
        overlap_route = None
        if (src_nw_addr,dst_nw_addr) ==(DEFAULT_ROUTE, DEFAULT_ROUTE):
            if (DEFAULT_ROUTE, DEFAULT_ROUTE) in self:
                overlap_route = self[DEFAULT_ROUTE,DEFAULT_ROUTE].route_id
        elif (src_nw_addr,dst_nw_addr) in self:
            overlap_route = self[(src_nw_addr,dst_nw_addr)].route_id

        if overlap_route is not None:
            msg = 'Destination overlaps [route_id=%d]' % overlap_route
            raise CommandFailure(msg=msg)

        routing_data = Route(route_id, dst_ip, dst_netmask, dst_vlan,gateway_ip,src_ip,src_netmask)
        src_ip_str=ip_addr_ntoa(src_ip)
        dst_ip_str = ip_addr_ntoa(dst_ip)
        key = '%s%d%s/%d' % (dst_ip_str,dst_netmask,src_ip_str, src_netmask)
        print "dstaddr: %s  added route %s"%(dst_nw_addr,routing_data)
...
class Route(object):
    def __init__(self, route_id, dst_ip, dst_netmask, dst_vlan, gateway_ip, src_ip, src_netmask):
    ...
    def __str__(self):
      return "id: %s, ip: %s, mask: %s"%(self.route_id, self.dst_ip, self.dst_netmask)
